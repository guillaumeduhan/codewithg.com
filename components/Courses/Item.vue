...
<template>
  <div class="relative mb-4 transition cursor-pointer CoursesItem hover:opacity-80" @click="redirect()
    ...
    <div class="flex flex-col gap-1">
      <p v-if="course.title" class="text-[16px] text-black font-bold mb-0" v-html="course.title" />
      <div v-if="!course.soon" class="flex text-sm items-center gap-2 description">
        {{ getPrice(course.price, locale) || "general.free" }}
      </div>
      <div v-else class="flex">
        <p class="description text-white font-bold bg-purple-500/80 rounded-xl px-2 py-1 text-[10px]">
